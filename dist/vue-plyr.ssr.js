...
var objectGetOwnPropertyNamesExternal$1 = {
  f: f$c
};var arraySlice = functionUncurryThis([].slice);var f$d = wellKnownSymbol$1;
...
var Array$2 = global_1$1.Array; // a part of `ArraySpeciesCreate` abstract operation
...
var arraySpeciesConstructor = function (originalArray) {
  ...
  if (isArray$1(originalArray)) {
    ...
    if (isConstructor(C) && (C === Array$2 || isArray$1(C.prototype))) C = undefined;else if (isObject$1(C)) {
      C = C[SPECIES$2];
      if (C === null) C = undefined;
    }
  }
...
hiddenKeys$2[HIDDEN$1] = true;var SPECIES$3 = wellKnownSymbol$1('species');
...
};var Array$3 = global_1$1.Array; // `Array.from` method implementation
...
handlePrototype(domTokenListPrototype);var defineProperty$9 = objectDefineProperty$1.f;
...
var URLSearchParamsState = function (init) {
  ...
  if (init !== undefined) {
    if (isObject$1(init)) this.parseObject(init);else this.parseQuery(typeof init == 'string' ? charAt$6(init, 0) === '?' ? stringSlice$9(init, 1) : init : toString_1(init));
  }
};
...
fixRegexpWellKnownSymbolLogic$1('replace', 2, function (REPLACE, nativeReplace, maybeCallNative, reason) {
  ...
  function (regexp, replaceValue) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      var position = max$6(min$9(toInteger(result.index), S.length), 0);
      var captures = []; // NOTE: This is equivalent to
      //   captures = result.slice(1).map(maybeToString)
      // but for some reason `nativeSlice.call(result, 1, result.length)` (called in
      // the slice polyfill when slicing native arrays) "doesn't work" in safari 9 and
...
var collectionStrong = {
  getConstructor: function (wrapper, CONSTRUCTOR_NAME, IS_MAP, ADDER) {
    ...
    if (descriptors) defineProperty$d(C.prototype, 'size', {
      get: function () {
...
_export({
  ...
  slice: function slice(start, end) {
    ...
    result = new (Constructor === undefined ? Array : Constructor)(max$7(fin - k, 0));
